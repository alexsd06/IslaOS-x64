...
unsigned int parse_ramdisk()
{
    ...
    uint64_t address = (uint64_t) ramdisk;
    unsigned int i;
    for (i = 0; ; i++)
    {
...
uint64_t get_pointer_to_file_header(char *filename)
{
    for (int i=0; i<ramdisk_cnt; i++) {
        if (strcmp(headers[i]->filename, filename)==0) {
            
            return ((uint64_t)headers[i]);
        }
    }
    return 0;
}
uint64_t get_pointer_to_file(char *filename)
{
    for (int i=0; i<ramdisk_cnt; i++) {
        if (strcmp(headers[i]->filename, filename)==0) {
...
void crdisk()
{
    kprintln("");
    tar_header *ramdisk=&_binary_ramdisk_tar_start;
    kprint ("The adress of the ramdisk is: "); kprintint((uint64_t)ramdisk); kprintln("");
    ...
    kprint ("The info header is at: "); kprintint((uint64_t) info_header); kprintln("");
